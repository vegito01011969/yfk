...
class KaggleYtDlpVideoDownloadProvider(ColabYtDlpVideoDownloadProvider):
    ...
    def _kernel_source(self, job: dict[str, Any], vendor_slug: str) -> str:
        ...
        injected = (
            ...
            "    vendor_mount = Path('/kaggle/input') / VENDOR_DATASET_SLUG\n"
            "    vendor_zip = vendor_mount / 'yt_dlp_vendor.zip'\n"
            "    if not vendor_zip.exists():\n"
            "        vendor_zip = vendor_mount / 'kaggle-yt-dlp-vendor.zip'\n"
            "    if not vendor_zip.exists():\n"
            "        matches = sorted(vendor_mount.glob('*.zip'))\n"
            "        if not vendor_mount.exists():\n"
            "            matches = []\n"
            "        if not matches:\n"
            "            matches = sorted(Path('/kaggle/input').glob('*/yt_dlp_vendor.zip'))\n"
            "        if not matches:\n"
            "            matches = sorted(\n"
            "                Path('/kaggle/input').glob('*/kaggle-yt-dlp-vendor.zip')\n"
            "            )\n"
            "        if not matches:\n"
            "            matches = sorted(Path('/kaggle/input').glob('*/*.zip'))\n"
            "        if not matches:\n"
            "            raise FileNotFoundError('yt-dlp vendor zip was not mounted by Kaggle')\n"
            "        vendor_zip = matches[0]\n"
            "    with zipfile.ZipFile(vendor_zip) as archive:\n"
            "        archive.extractall(vendor_dir)\n"
            "    sys.path.insert(0, str(vendor_dir))\n"
            "    os.environ['PYTHONPATH'] = (\n"
            "        str(vendor_dir) + os.pathsep + os.environ.get('PYTHONPATH', '')\n"
            "    )\n"
