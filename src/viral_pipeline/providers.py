...
class KaggleYtDlpVideoDownloadProvider(ColabYtDlpVideoDownloadProvider):
    def download_many(
        ...
        *,
        ...
        (kernel_dir / "kernel.py").write_text(
            self._kernel_source(job, vendor_slug),
            encoding="utf-8",
        )
    ...
    def _kernel_source(self, job: dict[str, Any], vendor_slug: str) -> str:
        ...
        injected = (
            ...
            f"VENDOR_DATASET_SLUG = {json.dumps(vendor_slug)}\n"
            "\n\n"
            "def _prepare_embedded_inputs() -> None:\n"
            "    input_root = Path('/kaggle/input')\n"
            "    vendor_dir = REMOTE_DIR / 'vendor'\n"
            "    vendor_dir.mkdir(parents=True, exist_ok=True)\n"
            "\n"
            "    def _input_listing() -> str:\n"
            "        if not input_root.exists():\n"
            "            return '/kaggle/input does not exist'\n"
            "        entries = sorted(input_root.rglob('*'))[:200]\n"
            "        if not entries:\n"
            "            return '/kaggle/input is empty'\n"
            "        return '\\n'.join(str(path.relative_to(input_root)) for path in entries)\n"
            "\n"
            "    def _package_parent(root: Path) -> Path | None:\n"
            "        if not root.exists():\n"
            "            return None\n"
            "        matches = sorted(\n"
            "            path\n"
            "            for path in root.rglob('__init__.py')\n"
            "            if path.parent.name == 'yt_dlp'\n"
            "        )\n"
            "        return matches[0].parent.parent if matches else None\n"
            "\n"
            "    def _extract_zip(zip_path: Path, destination: Path) -> bool:\n"
            ...
            "        return True\n"
            "\n"
            "    package_parent = _package_parent(input_root)\n"
            "    if package_parent is None:\n"
            "        if input_root.exists():\n"
            "            zip_candidates = sorted(input_root.rglob('*.zip'))\n"
            "        else:\n"
            "            zip_candidates = []\n"
            "        for index, zip_path in enumerate(zip_candidates):\n"
            "            extract_dir = vendor_dir / f'input_zip_{index}'\n"
            "            if not _extract_zip(zip_path, extract_dir):\n"
            "                continue\n"
            "            package_parent = _package_parent(extract_dir)\n"
            "            if package_parent is not None:\n"
            "                break\n"
            "        if package_parent is None:\n"
            "            nested_zips = sorted(vendor_dir.rglob('*.zip'))\n"
            "            for index, zip_path in enumerate(nested_zips):\n"
            "                extract_dir = vendor_dir / f'nested_zip_{index}'\n"
            "                if not _extract_zip(zip_path, extract_dir):\n"
            "                    continue\n"
            "                package_parent = _package_parent(extract_dir)\n"
            "                if package_parent is not None:\n"
            ...
            "    if package_parent is None:\n"
            "        raise FileNotFoundError(\n"
            "            'yt_dlp package was not found in Kaggle inputs. Mounted files:\\n'\n"
            "            + _input_listing()\n"
            "        )\n"
            "    sys.path.insert(0, str(package_parent))\n"
            "    os.environ['PYTHONPATH'] = (\n"
            "        str(package_parent) + os.pathsep + os.environ.get('PYTHONPATH', '')\n"
            "    )\n"
